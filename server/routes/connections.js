...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const row = await findExisting(req.user.id, otherId);
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const existing = await findExisting(req.user.id, toUserId);
    ...
    if (existing) {
      ...
      await db.run(
        `UPDATE connection_requests
         SET status = 'pending', from_user_id = ?, to_user_id = ?, created_at = NOW(), responded_at = NULL
         WHERE id = ?`,
        [req.user.id, toUserId, existing.id]
      );

      await createNotification({ userId: toUserId, type: 'connection_request', actorId: req.user.id });
      return res.status(201).json({ message: 'Request sent' });
    }

    const id = uuidv4();
    await db.run('INSERT INTO connection_requests (id, from_user_id, to_user_id) VALUES (?, ?, ?)', [
      id,
      req.user.id,
      toUserId
    ]);
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    const request = await db.get('SELECT * FROM connection_requests WHERE id = ?', [req.params.id]);
    ...
    await db.run(`UPDATE connection_requests SET status = ?, responded_at = NOW() WHERE id = ?`, [
      status,
      request.id
    ]);

    if (status === 'accepted') {
      await createNotification({ userId: request.from_user_id, type: 'connection_accepted', actorId: req.user.id });
    }
