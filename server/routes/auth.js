...
router.post('/signup', authLimiter, async (req, res) => {
  ...
  const existing = db
    .prepare('SELECT id FROM users WHERE (email_hash IS NOT NULL AND email_hash = ?) OR (phone IS NOT NULL AND phone = ?)')
    .get(emailHash, phone || null);
  if (existing) {
  ...
  db.prepare(
    `INSERT INTO users (id, email_hash, email_encrypted, phone, password_hash, display_name)
     VALUES (?, ?, ?, ?, ?, ?)`
  ).run(id, emailHash, emailEncrypted, phone || null, passwordHash, displayName);
...
router.post('/verify-otp', authLimiter, (req, res) => {
  ...
  const ok = verifyOtp(userId, code, 'verify');
  if (!ok) {
